...
type DirectGomokuInvitation={spaceId:string;roomId:string;wager:number;pot:number;expiresAt:number};
const DIRECT_GOMOKU_INVITE_PREFIX='[[POIO:GOMOKU:INVITE:1]]|';
function parseDirectGomokuInvitation(body:string):DirectGomokuInvitation|undefined{
  if(body.startsWith('[[POIO:GAME:INVITE:1]]|'))try{let encoded=body.slice(body.indexOf('|')+1).replace(/-/g,'+').replace(/_/g,'/');encoded+='='.repeat((4-encoded.length%4)%4);const bytes=Uint8Array.from(atob(encoded),character=>character.charCodeAt(0));const value=JSON.parse(new TextDecoder().decode(bytes));if(value.gameId==='gomoku'&&value.spaceId&&value.roomId)return {spaceId:value.spaceId,roomId:value.roomId,wager:Number(value.wager)||0,pot:Number(value.pot)||0,expiresAt:Number(value.expiresAt)||0};}catch{return undefined}
  ...
  return {spaceId,roomId,wager,pot,expiresAt};
}
const directMessagePreview=(body:string,attachmentName?:string)=>parseDirectGomokuInvitation(body)?'五子棋对局邀请':body||attachmentName||'附件';
...
export default function App(){
  ...
  const [pendingGomokuRoomId,setPendingGomokuRoomId]=useState('');
  ...
  const openGomokuInvitation=(spaceId:string,roomId:string)=>{const space=spaces.find(item=>item.id===spaceId);if(!space){showError(new Error('你已不在这个社区中'));return}setSpaceId(space.id);setChannelId(space.channels[0]?.id??'');localStorage.setItem(LAST_SPACE_KEY,space.id);setPendingGomokuRoomId(roomId);setGameOpen(true);setAccountOpen(false);setDirectPeer(undefined)};
  ...
  return <div className={`app-shell ${membersOpen?'':'members-hidden'} ${window.echodeck?.os==='darwin'?'macos':''}`}>
    ...
    {gameOpen&&currentSpace&&<GameCenter spaceId={currentSpace.id} spaceName={currentSpace.name} onlineMembers={onlineMembers} joinRoomId={pendingGomokuRoomId||undefined} onJoinRoomHandled={()=>setPendingGomokuRoomId('')} onClose={()=>setGameOpen(false)} onError={showError}/>}
...
function DirectMessageView({peer,currentUser,messages,onSend,onJoinGomoku,onError,onClose,onPreview}:{peer:User;currentUser:User;messages:DirectMessage[];onSend:(body:string,file?:File)=>Promise<void>;onJoinGomoku:(spaceId:string,roomId:string)=>void;onError:(error:unknown)=>void;onClose:()=>void;onPreview:(url:string,name:string)=>void}){
...
function DirectMessageRow({message,own,onJoinGomoku,onPreview}:{message:DirectMessage;own:boolean;onJoinGomoku:(spaceId:string,roomId:string)=>void;onPreview:(url:string,name:string)=>void}){const[copied,setCopied]=useState(false);const url=message.attachmentUrl?`${serverUrl}${message.attachmentUrl}`:'';const image=message.attachmentMime?.startsWith('image/');const invitation=parseDirectGomokuInvitation(message.body);const copy=async()=>{const text=invitation?'五子棋对局邀请':message.body||url||message.attachmentName||'';if(!text)return;await navigator.clipboard.writeText(text);setCopied(true);window.setTimeout(()=>setCopied(false),1200)};return <div className={`direct-message-row ${own?'own':''}`}><Avatar name={message.username} avatarUrl={message.avatarUrl}/><div><div className="direct-message-meta"><b>{own?'你':message.username}</b><time>{new Date(message.createdAt).toLocaleString('zh-CN')}</time><button title={copied?'已复制':'复制消息'} onClick={()=>void copy()}>{copied?<Check size={13}/>:<Copy size={13}/>}</button></div>{invitation?<DirectGomokuInviteCard invitation={invitation} own={own} onJoin={()=>onJoinGomoku(invitation.spaceId,invitation.roomId)}/>:message.body&&<MarkdownContent body={message.body}/>} {url&&(image?<button className="image-attachment" onClick={()=>onPreview(url,message.attachmentName??'图片')}><img src={url}/><span><Image size={13}/>{message.attachmentName}</span></button>:<a className="file-attachment" href={url} target="_blank" rel="noreferrer"><FileText/><span><b>{message.attachmentName}</b><small>{formatBytes(message.attachmentSize??0)}</small></span><Download size={17}/></a>)}</div></div>}
function DirectGomokuInviteCard({invitation,own,onJoin}:{invitation:DirectGomokuInvitation;own:boolean;onJoin:()=>void}){const expired=invitation.expiresAt<=Date.now();return <button className={`direct-gomoku-invite ${expired?'expired':''}`} disabled={expired} onClick={onJoin}><span className="direct-gomoku-icon"><Swords/></span><span><small>{own?'你发起了五子棋邀请':'五子棋对局邀请'}</small><b>{expired?'这张邀请已过期':own?'等待对方加入棋桌':'点击加入棋桌'}</b><em>每人 {invitation.wager.toLocaleString('zh-CN')} 积分 · 奖池 {invitation.pot.toLocaleString('zh-CN')}</em></span><Play/></button>}
