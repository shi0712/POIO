...
type DirectGameInvitation={gameId:'gomoku'|'texas-holdem';spaceId:string;roomId:string;wager:number;pot:number;expiresAt:number;metadata?:Record<string,unknown>};
...
function parseDirectGameInvitation(body:string):DirectGameInvitation|undefined{
  if(body.startsWith('[[POIO:GAME:INVITE:1]]|'))try{let encoded=body.slice(body.indexOf('|')+1).replace(/-/g,'+').replace(/_/g,'/');encoded+='='.repeat((4-encoded.length%4)%4);const bytes=Uint8Array.from(atob(encoded),character=>character.charCodeAt(0));const value=JSON.parse(new TextDecoder().decode(bytes));if(['gomoku','texas-holdem'].includes(value.gameId)&&value.spaceId&&value.roomId)return {gameId:value.gameId,spaceId:value.spaceId,roomId:value.roomId,wager:Number(value.wager)||0,pot:Number(value.pot)||0,expiresAt:Number(value.expiresAt)||0,metadata:value.metadata};}catch{return undefined}
...
const directMessagePreview=(body:string,attachmentName?:string)=>{const invitation=parseDirectGameInvitation(body);return invitation?(invitation.gameId==='gomoku'?'五子棋对局邀请':'德州扑克牌桌邀请'):body||attachmentName||'附件'};
...
export default function App(){
  ...
  const [pendingGameJoin,setPendingGameJoin]=useState<{gameId:'gomoku'|'texas-holdem';roomId:string}>();
  ...
  const openGameInvitation=(gameId:'gomoku'|'texas-holdem',spaceId:string,roomId:string)=>{const space=spaces.find(item=>item.id===spaceId);if(!space){showError(new Error('你已不在这个社区中'));return}setSpaceId(space.id);setChannelId(space.channels[0]?.id??'');localStorage.setItem(LAST_SPACE_KEY,space.id);setPendingGameJoin({gameId,roomId});setGameOpen(true);setAccountOpen(false);setDirectPeer(undefined)};
...
function DirectMessageView({peer,currentUser,messages,onSend,onJoinGame,onError,onClose,onPreview}:{peer:User;currentUser:User;messages:DirectMessage[];onSend:(body:string,file?:File)=>Promise<void>;onJoinGame:(gameId:'gomoku'|'texas-holdem',spaceId:string,roomId:string)=>void;onError:(error:unknown)=>void;onClose:()=>void;onPreview:(url:string,name:string)=>void}){
