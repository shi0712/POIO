...
import { BellRing, Check, ChevronDown, CirclePlus, Code2, Copy, Crown, Download, Eye, FileText, Gamepad2, Hash, HeadphoneOff, Headphones, Image, Keyboard, Link2, LogIn, LogOut, Maximize2, MessageCircle, MessageSquareOff, Mic, MicOff, Minimize2, Minus, MonitorUp, Pencil, PhoneOff, Play, Plus, Power, Quote, Save, Scissors, Search, Send, Settings, Shield, Smile, Square, Strikethrough, Trash2, Upload, UserMinus, UserPlus, Users, Volume2, VolumeX, X } from 'lucide-react';
...
export default function App(){
  ...
  return <div className={`app-shell ${membersOpen?'':'members-hidden'} ${window.echodeck?.os==='darwin'?'macos':''}`}>
    ...
    <aside className="space-rail"><div className="home active" title="POIO"><div className="mini-mark">P</div>{Object.values(directUnread).some(count=>count>0)&&<i className="space-unread"/>}</div><div className="rail-sep"/>{spaces.map(s=><button key={s.id} title={s.name} className={s.id===currentSpace?.id?'space active':'space'} onClick={()=>{const first=s.channels[0]?.id??'';setDirectPeer(undefined);setSpaceId(s.id);setChannelId(first);setUnreadChannels(current=>({...current,[first]:0}));setChannelQuery('');localStorage.setItem(LAST_SPACE_KEY,s.id)}}>{spaceInitial(s.name)}{s.channels.some(channel=>(unreadChannels[channel.id]??0)>0)&&<i className="space-unread"/>}<span>{s.name}</span></button>)}<button className="add-space" title="创建或加入社区" onClick={()=>setSpaceDialog('join')}><Plus/></button></aside>
...
function DirectMessageView({peer,currentUser,messages,onSend,onError,onClose,onPreview}:{peer:User;currentUser:User;messages:DirectMessage[];onSend:(body:string,file?:File)=>Promise<void>;onError:(error:unknown)=>void;onClose:()=>void;onPreview:(url:string,name:string)=>void}){
  ...
  return <section className="direct-message-view">
    ...
    <div ref={listRef} className="direct-message-list">{messages.length===0?<div className="direct-message-empty"><MessageCircle/><h2>开始与 {peer.username} 私聊</h2><p>你可以发送文字、图片和最大 50 MB 的文件。</p></div>:messages.map(message=><DirectMessageRow key={message.id} message={message} own={message.senderId===currentUser.id} onPreview={onPreview}/>)}</div>
