...
export default function App(){
  ...
  useEffect(()=>{
    ...
    return()=>{socket.off('game:gomoku:invited',received);window.clearTimeout(gomokuInvitationTimer.current)};
  },[user]);
  useEffect(()=>{
  ...
  const acceptGomokuInvitation=()=>{if(!gomokuInvitation)return;const space=spaces.find(item=>item.id===gomokuInvitation.spaceId);if(!space){showError(new Error('你已不在这个社区中'));setGomokuInvitation(undefined);return}setSpaceId(space.id);setChannelId(space.channels[0]?.id??'');localStorage.setItem(LAST_SPACE_KEY,space.id);setPendingGomokuRoomId(gomokuInvitation.roomId);setGameOpen(true);setAccountOpen(false);setGomokuInvitation(undefined)};
