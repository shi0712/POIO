...
try {
  ...
  const created=await request('channel:create',{spaceId:auth.bootstrap[0].id,name:'原生切换测试',kind:'voice'});const second=await request('voice:credentials',{channelId:created.id});
  ...
  const mute=await command('MUTE 1');const unmute=await command('MUTE 0');let move='';const moveDeadline=Date.now()+8000;do{move=await command(`MOVE ${second.channelName}`);if(move.startsWith('OK'))break;await new Promise(resolve=>setTimeout(resolve,400))}while(Date.now()<moveDeadline);
  ...
  console.log(JSON.stringify({nativeMumble:true,connected:true,port:first.port,firstChannel:first.channelName,secondChannel:second.channelName,inputDevices:devices.inputs.length,outputDevices:devices.outputs.length,configure,levelMin:Math.min(...levelSamples),levelMax:Math.max(...levelSamples),volumes,updatedVolumes,setInput,setOutput,mute,unmute,move}));
