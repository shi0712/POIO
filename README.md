...
> 当前版本：Windows **0.8.1 Preview**、Android **0.1.0-p25**。Windows 在线更新与官网下载均使用 ModelScope；安装包尚未进行代码签名，首次运行时可能显示 SmartScreen 提示。Android 当前提供 arm64 APK。
...
- 语音频道支持进入和离开提示音：进入者自己与频道内其他成员均可听到，支持账号级自定义加入音、试听、恢复默认和本地关闭；多开与短暂断线重连不会重复播放。
- 麦克风与听筒的关闭、恢复均使用独立提示音；听筒关闭遵循 Mumble 规则，同时关闭接收声音和麦克风。
...
- 频道聊天支持图片、文件、中文文件名、拖放/粘贴附件、区域截图、Markdown、GFM 和代码块；并支持回复、编辑、撤回、表情回应、@ 提醒、消息搜索、频道草稿和未读标记。
- 支持 PNG、JPG、WebP、GIF 自定义头像，GIF 可作为动态头像显示。
- 社区拥有者可以修改社区和频道名称、删除频道、踢出成员、禁止成员发送聊天消息，并通过 Mumble 服务端强制关闭成员麦克风。
...
- 客户端内检查更新、后台下载和重启安装。
- Android 版支持登录状态恢复、原生 Mumble 语音、语音房间内独立聊天入口、回复/编辑/撤回/表情回应/消息搜索、附件、动态头像、观看桌面共享（可独立控制共享声音）、断线状态卡、应用内下载进度与安装更新。
